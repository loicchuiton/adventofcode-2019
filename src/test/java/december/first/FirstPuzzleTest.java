...
class FirstPuzzleTest {
  private Logger logger = LogManager.getLogger(FirstPuzzleTest.class);

  @Test
  void test1() {
    Assertions.assertEquals(FirstPuzzle.proceed(12), 2);
  }

  @Test
  void test2() {
    Assertions.assertEquals(FirstPuzzle.proceed(14), 2);
  }

  @Test
  void test3() {
    Assertions.assertEquals(FirstPuzzle.proceed(1969), 654);
  }

  @Test
  void test4() {
    Assertions.assertEquals(FirstPuzzle.proceed(100756), 33583);
  }
  ...
  void finalTest() {
    double result =
        Stream.of(
            ...
            .map(FirstPuzzle::proceed)
            .reduce(Double.valueOf("0"), Double::sum);

    logger.debug("Result for 1st Puzzle is {}", result);

    // Added after for refactoring
    double expected = 3147032;
